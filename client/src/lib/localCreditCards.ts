...
export function creditCardFutureInstallmentCount(card: Pick<CreditCard, "purchases" | "isPaid" | "paidInvoices">, month: string) {
  return (card.purchases ?? []).filter(purchase => (purchase.installmentsTotal ?? 1) > 1 && Boolean(purchase.invoiceMonth) && purchase.invoiceMonth! > month && !creditCardIsInvoicePaid(card, purchase.invoiceMonth!)).length;
}
