...
import { createLocalInvestment, investmentCategories, investmentValue, loadLocalInvestments, saveLocalInvestments, type InvestmentCategory, type LocalInvestment } from "@/lib/localInvestments";
...
export default function Home() {
  ...
  return <div className="app-shell min-h-screen text-foreground">
    ...
    {investmentForm && <InvestmentModal form={investmentForm} setForm={setInvestmentForm} onClose={() => setInvestmentForm(null)} onSubmit={(event: React.FormEvent) => { event.preventDefault(); const numeric = Number(investmentForm.currentValue || (Number(investmentForm.quantity || 0) * Number(investmentForm.averagePrice || 0))); if (!investmentForm.name.trim() || !Number.isFinite(numeric) || numeric < 0) { toast.error("Preencha o nome e um valor válido"); return; } const nowIso = new Date().toISOString(); if (investmentForm.id) { setInvestments(current => current.map(item => item.id === investmentForm.id ? { ...item, ...investmentForm, currentValue: investmentForm.currentValue || String(numeric), updatedAt: nowIso } : item)); toast.success("Posição atualizada"); } else { setInvestments(current => createLocalInvestment(current, { ...investmentForm, userId: Number(user.id), currentValue: investmentForm.currentValue || String(numeric) })); toast.success("Posição adicionada"); } setInvestmentForm(null); }} />}
  </div>;
}
...
function Dashboard({ summary, transactions, allTransactions, month, year, selectedDate, performanceRange, onPerformanceRange, onSelectDate, onAdd, investments, onAddInvestment }: any) {
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      ...
      <InvestmentCard investments={investments} onAdd={onAddInvestment} />
    </div>
...
function InvestmentCard({ investments, onAdd }: any) { const total = investments.reduce((sum: number, item: LocalInvestment) => sum + investmentValue(item), 0); return <section className="glass-panel dashboard-card relative overflow-hidden rounded-3xl border p-5"><div className="absolute -right-10 -top-10 h-32 w-32 rounded-full bg-primary/15 blur-3xl" /><div className="flex items-start justify-between"><div><p className="text-sm font-medium">Carteira de investimentos</p><p className="mt-1 text-xs text-muted-foreground">Patrimônio acompanhado</p></div><BriefcaseBusiness className="h-5 w-5 text-primary" /></div><div className="mt-6"><p className="text-2xl font-semibold">{investments.length ? money.format(total) : "—"}</p><p className="mt-1 text-xs text-muted-foreground">{investments.length ? `${investments.length} posição(ões)` : "Nenhuma posição cadastrada"}</p></div><Button variant="secondary" size="sm" className="mt-5 w-full rounded-xl" onClick={onAdd}><Plus className="mr-2 h-4 w-4" />Cadastrar posição</Button></section>; }
