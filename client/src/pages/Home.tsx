...
import { createLocalInvestment, investmentCategories, loadLocalInvestments, saveLocalInvestments, type InvestmentCategory, type LocalInvestment } from "@/lib/localInvestments";
...
export default function Home() {
  ...
  return <div className="app-shell min-h-screen text-foreground">
    ...
    {investmentForm && <InvestmentModal form={investmentForm} setForm={setInvestmentForm} onClose={() => setInvestmentForm(null)} onSubmit={(event: React.FormEvent) => { event.preventDefault(); const numeric = Number(investmentForm.currentValue || investmentForm.quantity || 0); if (!investmentForm.name.trim() || !Number.isFinite(numeric) || numeric < 0) { toast.error("Preencha o nome e um valor válido"); return; } const nowIso = new Date().toISOString(); if (investmentForm.id) { setInvestments(current => current.map(item => item.id === investmentForm.id ? { ...item, ...investmentForm, currentValue: investmentForm.currentValue || "0", updatedAt: nowIso } : item)); toast.success("Posição atualizada"); } else { setInvestments(current => createLocalInvestment(current, { ...investmentForm, userId: Number(user.id), currentValue: investmentForm.currentValue || "0" })); toast.success("Posição adicionada"); } setInvestmentForm(null); }} />}
  </div>;
}
