...
import { ArrowDownRight, ArrowUpRight, BarChart3, BriefcaseBusiness, CalendarDays, ChevronDown, CircleArrowDown, CircleArrowUp, LayoutDashboard, LogOut, Plus, ReceiptText, Sparkles, Trash2, Wallet, X } from "lucide-react";
...
function Dashboard({ summary, transactions, allTransactions, month, year, selectedDate, onSelectDate, onAdd }: any) {
  ...
  const performance = buildPerformanceData(allTransactions, year);
  return <div className="space-y-6">
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      <Metric title="Saldo no período" value={money.format(summary.income - summary.expense)} icon={Wallet} accent="text-primary" helper="Resultado líquido" />
      <RankingCard title="Maiores gastos" items={biggestExpenses} empty="Nenhuma saída registrada" tone="expense" />
      <RankingCard title="Maiores entradas" items={biggestIncome} empty="Nenhuma entrada registrada" tone="income" />
      <InvestmentCard />
    </div>
    <div className="grid gap-4 xl:grid-cols-[1.1fr_.9fr]">
      <CalendarCard allTransactions={allTransactions} month={month} year={year} selectedDate={selectedDate} onSelectDate={onSelectDate} />
      <SelectedDayCard date={selectedDate} transactions={selectedTransactions} onAdd={onAdd} />
    </div>
...
function InvestmentCard() { return <section className="glass-panel dashboard-card relative overflow-hidden rounded-3xl border p-5"><div className="absolute -right-10 -top-10 h-32 w-32 rounded-full bg-primary/15 blur-3xl" /><div className="flex items-start justify-between"><div><p className="text-sm font-medium">Carteira de investimentos</p><p className="mt-1 text-xs text-muted-foreground">Patrimônio acompanhado</p></div><BriefcaseBusiness className="h-5 w-5 text-primary" /></div><div className="mt-6"><p className="text-2xl font-semibold">—</p><p className="mt-1 text-xs text-muted-foreground">Nenhuma posição cadastrada</p></div><div className="mt-5 rounded-2xl border border-dashed border-white/10 bg-white/[0.025] p-3 text-xs text-muted-foreground">Adicione posições quando quiser acompanhar sua carteira.</div></section>; }

function CalendarCard({ allTransactions, month, year, selectedDate, onSelectDate }: any) { const daysInMonth = new Date(year, month, 0).getDate(); const firstDay = (new Date(year, month - 1, 1).getDay() + 6) % 7; const byDate = new Map<string, LocalTransaction[]>(); allTransactions.forEach((item: LocalTransaction) => { const key = new Date(item.occurredAt).toISOString().slice(0, 10); byDate.set(key, [...(byDate.get(key) ?? []), item]); }); return <section className="glass-panel rounded-3xl border p-5 sm:p-6"><div className="flex items-start justify-between"><div><p className="text-sm font-medium">Calendário financeiro</p><p className="mt-1 text-xs text-muted-foreground">Clique em um dia para ver os lançamentos</p></div><CalendarDays className="h-5 w-5 text-primary" /></div><div className="mt-5 grid grid-cols-7 gap-1 text-center text-[11px] text-muted-foreground">{["S", "T", "Q", "Q", "S", "S", "D"].map((label, index) => <span key={`${label}-${index}`} className="py-1">{label}</span>)}{Array.from({ length: firstDay }).map((_, index) => <span key={`blank-${index}`} />)}{Array.from({ length: daysInMonth }, (_, index) => index + 1).map(day => { const key = `${year}-${String(month).padStart(2, "0")}-${String(day).padStart(2, "0")}`; const entries = byDate.get(key) ?? []; const active = selectedDate === key; return <button key={key} onClick={() => onSelectDate(key)} className={`relative grid aspect-square place-items-center rounded-xl text-sm transition ${active ? "bg-primary text-primary-foreground shadow-lg shadow-primary/20" : "hover:bg-white/8"}`}>{day}{entries.length > 0 && <span className={`absolute bottom-1 h-1 w-1 rounded-full ${entries.some(item => item.type === "income") ? "bg-emerald-400" : "bg-rose-400"}`} />}</button>; })}</div><div className="mt-4 flex items-center gap-4 text-[11px] text-muted-foreground"><span className="flex items-center gap-1.5"><i className="h-1.5 w-1.5 rounded-full bg-emerald-400" />Entradas</span><span className="flex items-center gap-1.5"><i className="h-1.5 w-1.5 rounded-full bg-rose-400" />Saídas</span></div></section>; }
...
function buildPerformanceData(transactions: LocalTransaction[], year: number) { let balance = 0; return monthNames.map((monthName, monthIndex) => { transactions.filter(item => { const date = new Date(item.occurredAt); return date.getFullYear() === year && date.getMonth() === monthIndex; }).forEach(item => { balance += item.type === "income" ? Number(item.amount) : -Number(item.amount); }); return { month: monthName, balance }; }); }

function PerformanceCard({ data }: any) { return <section className="glass-panel rounded-3xl border p-5 sm:p-6"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-sm font-medium">Performance financeira</p><p className="mt-1 text-xs text-muted-foreground">Evolução acumulada do saldo ao longo do ano</p></div><div className="rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2 text-xs text-muted-foreground">Ano atual</div></div><div className="mt-6 h-72"><ResponsiveContainer width="100%" height="100%"><AreaChart data={data}><defs><linearGradient id="performanceFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#c084fc" stopOpacity={0.38} /><stop offset="100%" stopColor="#c084fc" stopOpacity={0} /></linearGradient></defs><CartesianGrid stroke="rgba(255,255,255,.06)" vertical={false} /><XAxis dataKey="month" axisLine={false} tickLine={false} tick={{ fill: "#8a91a4", fontSize: 11 }} /><YAxis axisLine={false} tickLine={false} tick={{ fill: "#8a91a4", fontSize: 11 }} tickFormatter={value => `R$${Math.round(value / 1000)}k`} /><Tooltip contentStyle={{ background: "#17131f", border: "1px solid rgba(255,255,255,.12)", borderRadius: 14 }} formatter={(value: number) => money.format(value)} /><Area type="monotone" dataKey="balance" stroke="#d19aff" strokeWidth={3} fill="url(#performanceFill)" /></AreaChart></ResponsiveContainer></div></section>; }
